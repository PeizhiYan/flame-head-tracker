...
from submodules.flame_fitting.fitting_util import *
...
class Renderer(nn.Module):
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        attributes = torch.cat([self.face_uvcoords.expand(batch_size, -1, -1, -1), transformed_face_normals.detach(),
                                face_vertices_.detach(), face_normals.detach()], -1)
        # import ipdb;ipdb.set_trace()
    ...
    def render_shape(self, vertices, transformed_vertices, images=None, lights=None):
        ...
        # Attributes
        face_vertices = face_vertices(vertices, self.faces.expand(batch_size, -1, -1))
        normals = vertex_normals(vertices, self.faces.expand(batch_size, -1, -1));
        face_normals = face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1));
        transformed_face_normals = face_vertices(transformed_normals, self.faces.expand(batch_size, -1, -1))
    ...
    def render_normal(self, transformed_vertices, normals):
        ...
        # Attributes
        attributes = face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
    ...
    def world2uv(self, vertices):
        ...
        '''
        batch_size = vertices.shape[0]
        face_vertices = face_vertices(vertices, self.faces.expand(batch_size, -1, -1)).clone().detach()
