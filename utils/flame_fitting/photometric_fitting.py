...
from ..flame_lib.FLAME import FLAME, FLAMETex
from renderer import Renderer
from fitting_util import *# util
...
class PhotometricFitting(object):
    ...
    def _setup_renderer(self):
        mesh_file = './models/head_template.obj'
        self.render = Renderer(self.image_size, obj_filename=mesh_file).to(self.device)

    def optimize(self, images, landmarks, image_masks, params=None):
        # Modified by Peizhi
        # if params != None, use the given parameters and fit the 
        # non-given parameters
        bz = images.shape[0]
        if params is None:
            ...
            lights = torch.from_numpy(params['light']).to(self.device)
            cam = torch.zeros(bz, self.config.camera_params); cam[:, 0] = 5.
            cam = nn.Parameter(cam.float().to(self.device))
            e_opt_rigid = torch.optim.Adam(
                [cam],
        ...
        for k in range(200):
            ...
            trans_vertices = batch_orth_proj(vertices, cam);
            ...
            losses['landmark'] = l2_distance(landmarks2d[:, 17:, :2], gt_landmark[:, 17:, :2]) * self.config.w_lmks
        ...
        if params is None:
            ...
            for k in range(200, 1000):
                ...
                losses['landmark'] = l2_distance(landmarks2d[:, :, :2], gt_landmark[:, :, :2]) * self.config.w_lmks
        ...
        single_params = {
            ...
            'light': lights.detach().cpu().numpy()
        }
        return single_params, predicted_images

    def run_from_path(self, imagepath, landmarkpath):
        ...
        image_mask = image_mask.transpose(2, 0, 1) # CHW
        ...
        landmark[:, 0] = landmark[:, 0] / float(image.shape[2]) * 2 - 1 # normalize landmarks
        landmark[:, 1] = landmark[:, 1] / float(image.shape[1]) * 2 - 1 # normalize landmarks
        ...
        # optimize
        single_params, _ = self.optimize(images, landmarks, image_masks)
    ...
    def run(self, img, mask, landmark, params=None):
        # modified by Peizhi Yan
        # to fit the FLAME parameters on a single input image 
        #
        # inputs:
        #   - img: [H, W, C]   normalized to 0 ~ 1.0
        #   - mask: [H, W]
        #   - landmark: [68]
        # returns:
        #   - params: dictionary of FLAME parameters

        images = []
        landmarks = []
        image_masks = []

        image = cv2.resize(img, (self.config.cropped_size, self.config.cropped_size)).astype(np.float32)
        image = image[:, :, [2, 1, 0]].transpose(2, 0, 1)
        images.append(torch.from_numpy(image[None, :, :, :]).to(self.device))

        image_mask = np.array(mask)
        image_mask = image_mask[..., None].astype('float32')
        image_mask = image_mask.transpose(2, 0, 1) # CHW
        image_mask_bn = np.zeros_like(image_mask)
        image_mask_bn[np.where(image_mask != 0)] = 1.
        ...
        # optimize
        single_params, predicted_images = self.optimize(images, landmarks, image_masks, params)

        return single_params, predicted_images.detach().cpu().permute(0,2,3,1)[0].numpy()
...
if __name__ == '__main__':
    ...
    fitting.run_from_path(imagepath, landmarkpath)
